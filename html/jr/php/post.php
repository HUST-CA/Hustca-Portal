...
{
	echo "<script>alert('请输入姓名！');location.href= '../webapply.html'</script>";
	die(0);
}
elseif ($sex == "")
{
	echo "<script>alert('请选择性别！');location.href= '../webapply.html'</script>";
	die(0);
}
    elseif (!preg_match("/1[3458]{1}\d{9}$/",$tel))
{
	echo "<script>alert('请输入联系方式！');location.href= '../webapply.html'</script>";
	die(0);
}
elseif ($email == "")
{
	echo "<script>alert('请输入邮箱！');location.href= '../webapply.html'</script>";
	die(0);
}
else{
...
elseif (!preg_match("/[\x{4e00}-\x{9fa5}]{1,4}/u", $website))
{
	echo "<script>alert('请输入学院-专业-班级！');location.href= '../webapply.html'</script>";
	die(0);
}
elseif ($it_group == "")
{
	echo "<script>alert('请选择报名组别！');location.href= '../webapply.html'</script>";
	die(0);
}
elseif (!preg_match("/[\x{4e00}-\x{9fa5}]{1,4}/u", $experience))
{
	echo "<script>alert('请输入项目经历！');location.href= '../webapply.html'</script>";
	die(0);
}
elseif (!preg_match("/[\x{4e00}-\x{9fa5}]{1,4}/u", $excellent))
{
	echo "<script>alert('请输入擅长内容！');location.href= '../webapply.html'</script>";
	die(0);
}
}
